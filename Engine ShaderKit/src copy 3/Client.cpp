...
void Client::HandleMouseLook(float dt) {
    ...
    bool vIsPressed = (glfwGetKey(m_window, GLFW_KEY_V) == GLFW_PRESS);
    if (vIsPressed && !vWasPressed) {
        bool newState = !m_player.IsNoclip();
        m_player.SetNoclip(newState);
        Logger::Info("Noclip: " + std::string(newState ? "Enabled" : "Disabled"));
    }
    vWasPressed = vIsPressed;

    glm::vec3 forward, right;
    if (m_player.IsNoclip()) {
        // Full 3D vectors for flying
        forward = m_camera.GetForward();
        right   = m_camera.GetRight();
    } else {
        // Flattened vectors for walking
        forward = glm::normalize(glm::vec3(m_camera.GetForward().x, 0, m_camera.GetForward().z));
        right   = glm::normalize(glm::vec3(m_camera.GetRight().x, 0, m_camera.GetRight().z));
    }
