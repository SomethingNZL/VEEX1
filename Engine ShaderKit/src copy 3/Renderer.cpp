...
void Renderer::DrawMap(const BSP& map, const Camera& camera, int width, int height)
{
    ...
    // CHANGED: ask the BSP for only the visible batches given the camera position.
    // If VIS data is absent this returns the full batch list — no behaviour change.
    const auto& batches = map.GetVisibleBatches(camera.GetPosition());
    if (batches.empty()) return;

    // DEBUG: log when visible batch count changes so you can verify VIS is culling
    static int s_lastVisCount = -1;
    if ((int)batches.size() != s_lastVisCount) {
        s_lastVisCount = (int)batches.size();
        Logger::Info("VIS: visible_batches=" + std::to_string(batches.size()) +
                     " / total_batches=" + std::to_string(map.GetBatches().size()));
    }
