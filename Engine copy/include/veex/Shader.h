...
enum ShaderFeatureFlags : uint32_t {
    ...
    SHADER_FEATURE_DETAIL       = (1u << 10),  // → #define ENABLE_DETAIL 1 (detail texture)
    SHADER_FEATURE_EMISSIVE     = (1u << 11),  // → #define ENABLE_EMISSIVE 1 (emissive map)
};
...
struct MaterialParams {
    ...
    float bumpScale         = 1.0f;   // Normal map depth/bump scale ($bumpscale)
    
    // ── Silhouette Parallax Parameters ─────────────────────────────────────────
    float parallaxScale     = 0.03f;  // Height scale for parallax (typical 0.02-0.1)
    float parallaxMinLayers = 8.0f;   // Min ray-march layers (quality/perf tradeoff)
...
public:
    ...
    // Binds the "ProbeBlock" UBO at binding point 1 (call after Bind()).
    void BindProbeUBO(uint32_t uboHandle) const;
