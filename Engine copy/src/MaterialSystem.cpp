...
GLuint MaterialSystem::LoadTexture(const std::string& path, bool srgb) {
    ...
    if (channels != 4) {
        stbi_uc* rgbaData = new stbi_uc[width * height * 4];
        for (int i = 0; i < width * height; ++i) {
            rgbaData[i * 4 + 0] = data[i * channels + 0];
            rgbaData[i * 4 + 1] = (channels > 1) ? data[i * channels + 1] : 0;
            rgbaData[i * 4 + 2] = (channels > 2) ? data[i * channels + 2] : 0;
            rgbaData[i * 4 + 3] = 255;
        }
...
bool MaterialSystem::GetTextureData(GLuint textureID, std::vector<uint8_t>& outData,
                                   int& outWidth, int& outHeight, GLenum& outFormat) const {
    if (textureID == 0) {
        return false;
    }

    // Bind the texture to get its properties
    glBindTexture(GL_TEXTURE_2D, textureID);
    ...
    outFormat = static_cast<GLenum>(internalFormat);
    ...
    switch (outFormat) {
        ...
        case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
            isCompressed = true;
            // For compressed textures, we need to get the compressed size
            GLint compressedSize;
            glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_COMPRESSED_IMAGE_SIZE, &compressedSize);
            dataSize = static_cast<size_t>(compressedSize);
            break;
        
        case GL_RGB:
        case GL_RGB8:
        case GL_SRGB8:
            dataSize = outWidth * outHeight * 3;
            break;
        
        case GL_RGBA:
        case GL_RGBA8:
        case GL_SRGB8_ALPHA8:
            dataSize = outWidth * outHeight * 4;
            break;
        
        default:
            Logger::Error("Unsupported texture format for atlas packing: " + std::to_string(outFormat));
            glBindTexture(GL_TEXTURE_2D, 0);
            return false;
    }

    // Allocate buffer and read texture data
    outData.resize(dataSize);

    if (isCompressed) {
        glGetCompressedTexImage(GL_TEXTURE_2D, 0, outData.data());
    } else {
        glGetTexImage(GL_TEXTURE_2D, 0, outFormat, GL_UNSIGNED_BYTE, outData.data());
    }
