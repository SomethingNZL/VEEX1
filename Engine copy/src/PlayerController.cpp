...
PlayerController::PlayerController() 
    : m_velocity(0.0f), m_grounded(false), m_noclip(false), m_jumpHeld(false), m_loggedFirstGround(false) {}
...
void PlayerController::BuildCollisionMesh(const BSP& bsp)
{
    ...
    // Flags for non-collidable surfaces (water, sky, etc)
    ...
    for (const auto& face : faces)
    {
        ...
        for (size_t i = 1; i + 1 < faceVerts.size(); ++i) {
            ...
            // Double-sided collision to prevent falling through thin brushes
    ...
    Logger::Info("PlayerController: Collision mesh ready with " + std::to_string(m_tris.size()) + " triangles.");
}
...
{
    const glm::vec3 he(kHullHW, kHullHH, kHullHW);
    const glm::vec3 center = feetPos + glm::vec3(0.0f, kHullHH, 0.0f);
...
{
    // If moving upwards fast (e.g. just jumped), we aren't grounded
    if (m_velocity.y > (kJumpImpulse * 0.5f)) {
        m_grounded = false;
        return;
    }
    ...
    if (tr.hit && tr.hitNormal.y >= kSlopeLimit) {
        ...
        if (!m_loggedFirstGround) {
            ...
            Logger::Info("PlayerController: Grounded at Y = " + std::to_string(pos.y));
        }
    } else {
        m_grounded = false;
    }
                                  ...
                                  float wishSpeed, bool jumpPressed, float dt)
{
    if (m_noclip) {
        m_grounded = false;
        m_velocity = glm::vec3(0.0f);
        return feetPos + (wishDir * (wishSpeed * kScale) * dt);
    }
    ...
    // 1. Friction
    if (m_grounded) {
        float speed = glm::length(glm::vec3(m_velocity.x, 0.0f, m_velocity.z));
        if (speed > 0.01f) {
    ...
    // 4. Gravity (Half-step 1)
    if (!m_grounded) m_velocity.y -= kGravity * dt * 0.5f;
    
    // 5. Kinematics
    if (m_grounded) StepMove(pos, dt);
    else            TryPlayerMove(pos, m_velocity, dt);
    
    // 6. Gravity (Half-step 2)
    if (!m_grounded) m_velocity.y -= kGravity * dt * 0.5f;
