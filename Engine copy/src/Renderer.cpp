...
// Converted to forward rendering with advanced shading models (Oren-Nayar, Blinn-Phong, Fresnel).
...
bool Renderer::Init(const GameInfo& game)
{
    ...
    // RNM: 8 vertex attributes for 3 lightmap UV sets + face normal
    ...
    glEnableVertexAttribArray(3); // LightmapCoord0 (RNM basis 0)
    glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, lmCoord0));
    glEnableVertexAttribArray(4); // Tangent
    glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, tangent));
    glEnableVertexAttribArray(5); // LightmapCoord1 (RNM basis 1)
    glVertexAttribPointer(5, 2, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, lmCoord1));
    glEnableVertexAttribArray(6); // LightmapCoord2 (RNM basis 2)
    glVertexAttribPointer(6, 2, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, lmCoord2));
    glEnableVertexAttribArray(7); // FaceNormal (for RNM basis orientation)
    glVertexAttribPointer(7, 3, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, faceNormal));
    ...
    // Black heightmap = zero displacement (no parallax effect when missing)
    m_heightDefault    = MakeFallbackTexture(0, 0, 0);
...
void Renderer::DrawGBufferPass(const BSP& map, const Camera& camera, int width, int height)
{
    ...
    m_bspShader.BindSampler("u_HeightTexture",   9);
    ...
    for (const auto& batch : map.GetBatches()) {
        ...
        glBindTexture(GL_TEXTURE_2D, batch.key.heightID ? batch.key.heightID : m_heightDefault);
        ...
        mp.enableSilhouetteParallax = batch.key.heightID ? true : false;
        mp.parallaxScale     = batch.matParams.parallaxScale;
        mp.parallaxMinLayers = batch.matParams.parallaxMinLayers;
        mp.parallaxMaxLayers = batch.matParams.parallaxMaxLayers;
...
void Renderer::DrawMapInternal(const BSP& map, const Camera& camera, int width, int height)
{
    ...
    if (probeUBO) {
        m_bspShader.BindProbeUBO(probeUBO);
    }
    ...
    // Bind height/displacement texture sampler
    m_bspShader.BindSampler("u_HeightTexture",    9);
    ...
    for (const auto& batch : batches) {
        ...
        // Bind height/displacement texture
        glActiveTexture(GL_TEXTURE9);
        glBindTexture(GL_TEXTURE_2D, batch.key.heightID ? batch.key.heightID : m_heightDefault);
        ...
        mp.emissiveScale   = batch.matParams.emissiveScale;
        mp.hasNormalMap    = batch.normalID() ? true : false;
        mp.enableRNM       = mp.hasNormalMap;  // Enable RNM when normal map is present
        mp.hasRoughnessMap = batch.roughnessID() ? true : false;
        mp.hasMetallicMap  = batch.metallicID() ? true : false;
        mp.hasSpecMaskMap  = batch.key.specMaskID ? true : false;
        mp.hasEmissiveMap  = batch.key.emissiveID ? true : false;
        ...
        // Enable parallax if a height map is present
        mp.enableSilhouetteParallax = batch.key.heightID ? true : false;
        mp.parallaxScale     = batch.matParams.parallaxScale;
        mp.parallaxMinLayers = batch.matParams.parallaxMinLayers;
        mp.parallaxMaxLayers = batch.matParams.parallaxMaxLayers;
        ...
        mp.useHeightAtlas    = batch.matParams.useHeightAtlas;
        mp.heightAtlasUVCrop = batch.matParams.heightAtlasUVCrop;

        // ── Atlas Support ────────────────────────────────────────────────────────
        // Check if this batch uses atlas textures
        bool useAtlas = false;
        glm::vec4 atlasCrop(0.0f, 0.0f, 1.0f, 1.0f);
        
        // For now, we'll need to check the material system to see if this texture uses atlas
        // This is a placeholder - in a real implementation, the batch would store atlas info
        // For testing, we'll assume atlas is not used until properly integrated
        useAtlas = false;
        ...
        // Set bumpScale from VMT data (default 1.0 if not specified)
        mp.bumpScale         = batch.matParams.bumpScale;
...
void Renderer::Shutdown()
{
    ...
    if (m_heightDefault && glad_glDeleteTextures)    glDeleteTextures(1, &m_heightDefault);
