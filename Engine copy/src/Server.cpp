...
// Include the implementation inside the namespace so Server can see the class definition
...
bool Server::Init(const std::string& entityData, const std::string& binfoPath, const GameInfo& game)
{
    ...
    // Set global GameInfo for entity access
    Entity::SetGameInfo(&game);
...
bool Server::SpawnEntity(const EntityData& ed)
{
    ...
    if (classname == "ambient_generic") {
        newEnt = std::make_shared<AmbientGeneric>(m_nextEntityID++, classname);
    } 
    else {
        // Use the factory if the class is registered
        newEnt = EntityFactory::Create(classname, m_nextEntityID++);
    }
    
    // Fallback to generic entity if not in factory and not a special type
    if (!newEnt) {
        newEnt = std::make_shared<Entity>(m_nextEntityID - 1, classname);
    }

    // Handle spawn point tracking
    ...
    if (newEnt) {
        ...
        // Call Spawn() for entity-specific initialization (model loading, etc.)
        if (!newEnt->Spawn(ed)) {
            Logger::Warn("Server: Entity '" + classname + "' Spawn() returned false, discarding.");
            return false;
        }

        // Trigger the custom spawn logic (initializes sounds, etc)
        newEnt->OnSpawn();
