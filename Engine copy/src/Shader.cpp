...
void Shader::BindProbeUBO(uint32_t uboHandle) const
{
    if (!m_program || !uboHandle) return;
    uint32_t blockIdx = glGetUniformBlockIndex(m_program, "ProbeBlock");
    if (blockIdx != GL_INVALID_INDEX)
        glUniformBlockBinding(m_program, blockIdx, 1);
...
void Shader::UploadMaterialParams(const MaterialParams& mat)
{
    ...
    SetInt  ("u_EnableRNM",       mat.enableRNM       ? 1 : 0);  // RNM uniform
    SetInt  ("u_EnableSilhouetteParallax", mat.enableSilhouetteParallax ? 1 : 0);
    SetFloat("u_NormalMapStrength", mat.normalMapStrength);
    ...
    SetFloat("u_BumpScale", mat.bumpScale);

    // ── Silhouette Parallax Parameters ─────────────────────────────────────────
    SetFloat("u_ParallaxScale",     mat.parallaxScale);
    SetFloat("u_ParallaxMinLayers", mat.parallaxMinLayers);
    SetFloat("u_ParallaxMaxLayers", mat.parallaxMaxLayers);
    
    // ── Height Map Atlas Parameters ────────────────────────────────────────────
    SetVec4("u_HeightAtlasCrop", mat.heightAtlasUVCrop.x, mat.heightAtlasUVCrop.y,
            mat.heightAtlasUVCrop.z, mat.heightAtlasUVCrop.w);
    SetInt  ("u_UseHeightAtlas", mat.useHeightAtlas ? 1 : 0);
}
