...
struct VPKFileEntry {
    ...
    uint32_t archiveIndex;      ///< Index of the archive file (0xFFFF for directory)
 ...
 * 
 * Supports VPK version 2 format used by modern Source Engine games.
...
public:
     ...
     * @return True if successful.
     */
    bool GetAllFiles(std::vector<std::string>& files) const;

private:
    struct VPKHeader {
        uint32_t signature;     ///< VPK signature (should be 0x55aa1234)
        uint32_t version;       ///< VPK version
        uint32_t directorySize; ///< Size of the directory
        uint32_t archiveCount;  ///< Number of archive files
        uint32_t chunkSize;     ///< Size of each chunk (for version 2)
        uint32_t dummy;         ///< Unused field
    };
    
    std::string vpkPath;                    ///< Path to the VPK file
    mutable std::ifstream file;             ///< File stream for reading (mutable for const methods)
    VPKHeader header;                       ///< VPK header
    ...
    /**
     * @brief Read preload data for a file.
     * @param entry File entry to read preload data for.
     * @return True if successful.
     */
    bool ReadPreloadData(VPKFileEntry* entry);
