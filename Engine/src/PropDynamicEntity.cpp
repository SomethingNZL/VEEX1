...
bool PropDynamicEntity::Spawn(const EntityData& ed) {
    ...
    if (m_modelPath.size() > 4 && 
        ...
        // Need GameInfo for MDL loading - get it from somewhere
        // For now, we'll use a simple approach - the Server should provide this
        // In a real implementation, we'd pass GameInfo through EntityData or Server
        Logger::Warn("PropDynamicEntity: MDL loading requires GameInfo - model not loaded yet");
        // Model will be loaded later when GameInfo is available
    } else {
    ...
    if (!m_modelPath.empty() && m_model) {
        ...
        if (mdlModel && m_currentSequence >= 0 && m_currentSequence < mdlModel->GetNumSequences()) {
...
void PropDynamicEntity::Update(float dt) {
    ...
    // Check if we need to reload the model (if it wasn't loaded at spawn)
    if (!m_model->GetMDLModel() && !m_cachedModelPath.empty()) {
        // Try to load with MDL cache
        // This would need GameInfo passed through somehow
