...
void ReflectionProbeSystem::CaptureProbe(ReflectionProbe* probe, const BSP& map, 
    ...
    for (int i = 0; i < 6; i++) {
        ...
        // Create a temporary camera at the probe position with the face's view matrix
        glm::vec3 forward = glm::normalize(glm::vec3(captureViews[i][0][2], captureViews[i][1][2], captureViews[i][2][2]));
        glm::vec3 right = glm::normalize(glm::vec3(captureViews[i][0][0], captureViews[i][1][0], captureViews[i][2][0]));
        glm::vec3 up = glm::normalize(glm::vec3(captureViews[i][0][1], captureViews[i][1][1], captureViews[i][2][1]));
        
        // Calculate yaw and pitch from the forward vector
        float yaw = atan2(forward.x, forward.z);
        float pitch = asin(forward.y);
        
        // Create a temporary camera for this face
        Camera faceCamera;
        faceCamera.SetPosition(position);
        faceCamera.SetRotation(glm::degrees(yaw), glm::degrees(pitch));
        
        // Create view-projection matrix for this face
        glm::mat4 faceViewProj = captureProj * captureViews[i];
        
        // The actual rendering should be done by the Renderer's RenderReflectionProbeCubemap method
        // which is called externally by the Renderer. For now, we just clear to a debug color
        // to show the cubemap faces are being captured.
        // In a full implementation, the Renderer would call back into this system to render each face.
